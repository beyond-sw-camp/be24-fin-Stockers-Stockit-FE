...
const activeSideMenu = ref('재고 현황')
...
const mergedInventory = computed(() => {
  const stockMap = Object.fromEntries(
    inventoryStore.products.map(p => [p.id, p.stock])
  )
  return inventoryData.map(item => {
    if (stockMap[item.id] !== undefined) {
      const s = stockMap[item.id]
      const safety = item.safety
      const status = s === 0 ? '품절' : s <= safety ? '부족' : '정상'
      return { ...item, current: s, avail: s, status }
    }
    return item
  })
})
...
const filteredInventory = computed(() => {
  ...
  if (!keyword) return mergedInventory.value
  return mergedInventory.value.filter((item) => {
    const target = [item.id, item.name, item.cat, item.loc].join(' ').toLowerCase()
    return target.includes(keyword)
  })
})

const selectItem = (row) => { selectedItem.value = row }
const clearSelectedItem = () => { selectedItem.value = null }
      ...
      <section class="panel filter-bar">
        <div class="filter-group">
          ...
          <label class="search-box search-inline">
            ...
            <input v-model="searchTerm" type="text" placeholder="품목 코드 또는 품목명 검색..." />
          </label>
        </div>
      ...
      <section class="inventory-split">
        <div class="panel table-panel">
          ...
          <div class="table-footer">
            ...
            <div class="pagination">
              <button type="button" class="nav-icon"><ChevronLeftIcon :size="14" /></button>
              <button v-for="page in [1, 2, 3, '...', 48]" :key="page" type="button" :class="{ active: page === 1 }">{{ page }}</button>
              <button type="button" class="nav-icon"><ChevronRightIcon :size="14" /></button>
            </div>
          </div>
        </div>
        ...
        <aside v-if="selectedItem" class="panel detail-panel">
          <div class="detail-head">
            <h3><InfoIcon :size="14" /> 품목 상세 정보</h3>
...
.inventory-table .narrow { width: 96px; }
.inventory-table .medium { width: 128px; }
.inventory-table .wide { width: 144px; }

.center { text-align: center !important; }
.align-right { text-align: right !important; }
.strong { color: #1f2937 !important; font-size: 12px !important; font-weight: 900; }
.semi-strong { color: #4b5563 !important; font-weight: 700; }
.muted { color: #9ca3af !important; }
.truncate { overflow: hidden; text-overflow: ellipsis; }
.strong-small { font-weight: 700; }
.strong-number { color: #111827; font-size: 12px !important; font-weight: 900; }
.brand-text { color: #004d3c !important; }
.faded { color: #d1d5db !important; }
...
.status-dot.정상 { background: #10b981; }
.status-dot.부족 { background: #ef4444; }
.status-dot.임박 { background: #f59e0b; }
.status-dot.품절 { background: #9ca3af; }
.status-text.정상 { color: #047857; }
.status-text.부족 { color: #b91c1c; }
.status-text.임박 { color: #b45309; }
.status-text.품절 { color: #6b7280; }
...
.meter-fill.부족,
.meter-fill.품절 { background: #dc2626; }
.meter-fill.임박 { background: #f59e0b; }
...
.history-bar.current { background: #004d3c; }
...
@media (max-width: 1180px) {
  .inventory-split { flex-direction: column; }
