...
const BASE = '/api/warehouse/inventories'

// 창고 재고 마스터(품목 단위) 페이지 조회.
// params: { parentCategory?, childCategory?, category?, status?, keyword?, page, size }
...
export async function getWarehouseInventorySkus(params = {}) {
  const res = await api.get(`${BASE}/skus`, { params })
  return unwrap(res)
}

// 창고 재고 SKU 칩 필터용 facets — 거점/카테고리/검색 조건 안의 가능한 색상/사이즈 distinct.
// 응답: { colors: [...], sizes: [...] }
export async function getWarehouseInventorySkuFacets(params = {}) {
