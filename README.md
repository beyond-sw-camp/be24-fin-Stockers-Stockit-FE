...
패스트패션 산업에서 매년 수백만 벌의 의류가 소각되는 현실 속에서, STOCKIT은 단순한 ERP를 넘어재고를 *버려야 할 쓰레기*가 아닌 **다시 순환될 수 있는 새로운 자원**으로 바라보고 **재고를 자산으로 전환**하는 새로운 패러다임을 제시합니다. 발주·출고·입고·판매·순환(업사이클링)에 이르는 재고의 라이프사이클 전 과정을 단일 플랫폼에서 추적·관리합니다.
...
> 🎨 [화면 설계서 (Figma)](https://www.figma.com/design/zDcSTD8tNvZb3m5yjZPurb/%EC%A0%9C%EB%AA%A9-%EC%97%86%EC%9D%8C?node-id=0-1&t=LZgIMCGwJqqVEE7R-1)
...
📦 재고량 기준 → 현재 재고가 월 평균 판매량의 일정 배수 초과 시 경고
💹 회전율 기준 → 카테고리 평균 대비 현저히 낮은 회전율 품목 탐지
```
...
```
소재 정보 벡터화  ──┐
                    ├──→ 유사도 연산 ──→ 최적 거래처 Top-N 추천
거래처 DB 벡터화  ──┘    (Cosine Similarity)
  (공장현황 + 사회적기업 목록 + KSIC 11차 분류)
```

- Elasticsearch 기반 **소재·조건 필터 검색** (면 70% 이상 등 조건 검색)
- 카테고리 / 서브카테고리 정밀 필터링
...
**탄소 감축량 산출 방식 (Net-Impact 엔진)**

```
탄소 감축량 = 소각 회피 탄소량(GIR 소각계수) + 소재 내재 탄소가치(Higg MSI) - 물류 배출량(DEFRA 계수)
```

- **소재별 탄소 계수** (Higg MSI 기준): Cotton 1.8 / Polyester 2.3 / Nylon 2.5 kg CO₂e/kg
- KAU 시세 기준 경제적 가치 환산 (예: 면 5,000kg 순환 → 4.5 tCO₂ → 약 44,280원)
- GRI 306-4, GHG Protocol, SASB 공시 표준 자동 매핑
...
- **재고 상태 자동 태깅**: `과잉` / `정상` / `부족` / `품절` 4단계 자동 판별
- **Kafka 이벤트 드리븐**: 입출고 이벤트 실시간 반영
...
| 탄소 사전할당량 | 기후에너지환경부 온실가스종합정보센터 | 기업별 배출 한도 기준선 설정 |
...
외부 연동
├── 금융위 배출권 API   (KAU 시세 실시간 연동)
├── GIR 소각 탄소계수   (탄소 감축량 산정)
└── Higg MSI           (소재별 탄소 발자국)
```
...
> 💡 아래 자리에 Jenkins 파이프라인, ArgoCD 대시보드, K8s 파드 전환 화면 캡처를 삽입해 주세요.

```
[Jenkins_파이프라인_성공_캡처.png]

[ArgoCD_Sync_상태_캡처.png]

[K8s_BlueGreen_파드_전환_캡처.png]
```

| 단계 | 화면 | 설명 |
|------|------|------|
| Jenkins Build | `[캡처 삽입]` | 빌드·테스트·이미지 푸시 파이프라인 전체 성공 |
| ArgoCD Sync | `[캡처 삽입]` | GitOps 기반 K8s 배포 동기화 상태 (Synced / Healthy) |
| 트래픽 전환 | `[캡처 삽입]` | Blue → Green 무중단 전환 완료, 파드 상태 확인 |
| 롤백 | `[캡처 삽입]` | 이상 감지 시 Green → Blue 즉시 롤백 |
